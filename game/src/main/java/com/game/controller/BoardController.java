...
///////////////////////////////////////////////////////////공지사항 테스트중 Start
     ...
     * openNoticeBoardList(@ModelAttribute("params") BoardDTO params, Model model) {
     * // 메인 생성되면 보드타입 변경? params.setBoardType(2); List<BoardDTO> boardList =
     * boardService.getBoardList(params); model.addAttribute("boardList",
     * boardList);
     * 
     * return "admin/list2"; }
     * 
     * @GetMapping(value = "/noticeboard/view") public String
     * openNoticeBoardDetail(@ModelAttribute("params") BoardDTO params, Model model)
     * { System.out.println("boardNum:" + params.getBoardNum()); if
     * (params.getBoardNum() == null) { return
     * showMessageWithRedirect("올바르지 않은 접근입니다. 목록화면으로 이동합니다,",
     * "/board/noticeboard/list", Method.GET, null, model); } BoardDTO board =
     * boardService.getBoardDetail(params); if (board == null ||
     * "Y".equals(board.getBoardDelete())) { return
     * showMessageWithRedirect("없거나 이미 삭제된 게시글입니다. 목록화면으로 이동합니다,",
     * "/board/noticeboard/list", Method.GET, null, model); }
     * model.addAttribute("board", board);
     * 
     * return "admin/view2"; } <<<<<<< HEAD
     * 
     * =======
     */
