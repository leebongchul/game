...
///////////////////////////////////////////////////////////신고하기
	...
	public String openBoardDetail(@ModelAttribute("params") BoardDTO params, Model model) {
		System.out.println("boardNum:" + params.getBoardNum());
		if (params.getBoardNum() == null) {
			return showMessageWithRedirect("올바르지 않은 접근입니다. 목록화면으로 이동합니다.", "/board/list", Method.GET, null,
					model);
		}
		BoardDTO board = boardService.getBoardDetail(params);
		if (board.getBoardNum() == null || "Y".equals(board.getBoardDelete())) {
			return showMessageWithRedirect("없거나 이미 삭제된 게시글입니다. 목록화면으로 이동합니다.", "/board/list", Method.GET,
					null, model);
		}
...
//		return "board/view";
	...
	public String registerBoard(@ModelAttribute("params") BoardDTO params, Model model) {
		...
		try {
			...
			return showMessageWithRedirect("데이터베이스 처리 과정에 문제가 발생하였습니다.", "/board/list", Method.GET, null,
					model);
...
//		params.setMemRole(loginMember.getMemRole());
		...
		try {
			if (params.getBoardUpdateId() == params.getMemId() || params.getMemRole() == "admin") {
				...
				if (isDeleted == false) {
					return showMessageWithRedirect("게시글 삭제에 실패하였습니다.", "/board/list", Method.GET, null,
							model);
				}
			} else {
